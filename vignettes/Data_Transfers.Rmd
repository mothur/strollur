...
The *rdataset* package stores the data associated with your microbial DNA analysis. This tutorial will explain how to save, load, copy, export, and import your dataset object. If you haven't reviewed the [Getting Started](Getting-Started.html) tuturial, we recommend you start there. 
...
The rdataset package has a function to save a dataset object as an *.rds* file, `save_dataset()`, and a function to create a dataset from an *.rds* file, `load_dataset()`. Let's use the miseq data object to learn how to do that.
...
```{r}
dgc_data <- read_mothur_list(list = rdataset_example("final.dgc.list.gz"))
