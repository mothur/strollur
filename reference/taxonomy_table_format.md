...
The `full` taxonomy table is a `data.frame` with two columns:
`sequence_name` or `bin_name`, and `taxonomy`. The `taxonomy` column
contains the full classification string.
...
The `tidy` taxonomy table is a `data.frame` with 4 columns:
`sequence_name` or `bin_name`, `taxonomy`, `level`, and `confidence`.
For example:
